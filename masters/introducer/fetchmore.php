...
$getData = mysql_query("SELECT introducer.name,introducer.modifieddate,employee.name,introducer.id FROM introducer,employee WHERE introducer.updatedby = employee.id AND introducer.delete = '0' AND introducer.id != '1' AND introducer.id < '$data[0]' ORDER BY introducer.id DESC LIMIT ".$fc,$con) or die(mysql_error());
...
<td class="blueSimpletext" style="width:300px;" onclick="getModule('masters/introducer/edit?id=<?php echo $row[3];?>&i=<?php echo $i;?>','manipulateContent','viewContent','Fetching Data..')"><?php echo $row[0];?></td>
