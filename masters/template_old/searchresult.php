...
$getData = mysql_query("SELECT template.name,template.modifieddate,employee.name,template.id,template.template,template.messenger FROM template,employee WHERE template.updatedby = employee.id AND template.delete = '0' AND template.id != '1' AND template.template LIKE '$term%' ORDER BY template.template ",$con) or die(mysql_error());
...
?>
	<tr id="fetchRow<?php echo $i;?>" class="d<?php echo $i%2;?>">
		...
		<td onclick="getModule('masters/template/edit?id=<?php echo $row[3];?>&amp;i=<?php echo $i;?>','manipulateContent','viewContent','Fetching Data..')" style="width: 300px;">
		<?php echo $row[0];?></td>
