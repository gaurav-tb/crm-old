...
$getData = mysql_query("SELECT leadsource.name,leadsource.modifieddate,employee.name,leadsource.id,leadsource.description FROM leadsource,employee WHERE leadsource.updatedby = employee.id AND leadsource.delete = '0' AND leadsource.id != '1' AND leadsource.id < '$data[0]' ORDER BY leadsource.id DESC LIMIT ".$fc,$con) or die(mysql_error());
...
<td class="blueSimpletext" style="width:300px;" onclick="getModule('masters/leadsource/edit?id=<?php echo $row[3];?>&i=<?php echo $i;?>','manipulateContent','viewContent','Fetching Data..')"><?php echo $row[0];?></td>
