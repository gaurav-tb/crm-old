...
{
$sql = "SELECT employee.name, contact.fname, contact.mobile, contact.callbackdate,contact.id,leadresponse.name,contact.modifieddate,contact.mark,contact.read,contact.lname FROM contact,employee,leadresponse WHERE contact.ownerid = employee.id AND contact.delete = '0' AND contact.converted= '0' AND contact.latestresponse = leadresponse.id".$leadstr." ".$sortstr;
$getData = mysql_query($sql,$con) or die(mysql_error());
}
else
{
$sql = "SELECT employee.name, contact.fname, contact.mobile, contact.callbackdate,contact.id,leadresponse.name,contact.modifieddate,contact.mark,contact.read,contact.lname FROM contact,employee,leadresponse WHERE contact.ownerid = employee.id AND contact.delete = '0' AND contact.converted= '0' AND contact.latestresponse = leadresponse.id AND contact.ownerid = '$loggeduserid'".$leadstr." ".$sortstr;;
$getData = mysql_query($sql,$con) or die(mysql_error());
}
...
?>
	<tr <?php if($row[8] == '0') echo "style='font-weight:bold'"; ?> id="fetchRow<?php echo $i;?>" class="d<?php echo $i%2;?>" title="<?php echo "Last Updated on ".date("d-m-Y H:i:s",strtotime($row[6]));?>">
		...
		<?php echo $row[0];?></td>
		<td onclick="getModule('leads/edit?id=<?php echo $row[4];?>&amp;i=<?php echo $i;?>','manipulateContent','viewContent','<?php echo $row[1];?>')"  class="blueSimpletext"  style="width: 200px;"><?php echo $row[1];?></td>
		<td style="width:100px"><?php echo $row[9];?></td>
		...
		?>
			...
			<td   style="width: 200px;">
		<?php echo date("d,M y H:i",strtotime($row[6]));?>
		</td>
		<td>
