...
// ORDER BY  `email_queue`.`queue_up_date` ASC",$con) or die(mysql_error());
$getData=mysql_query("call getTemplate()");
...
//echo "SELECT `Segments`,`AmountWithGst`,`StartDate`,`EndDate`,`Activationamt` FROM `researchbooster` WHERE `cid`='$cid' AND `RequestingDate`='$date'";
	// $getBooster=mysql_query("call getBooster($cid,'$date')");
	// $rowBooster= mysql_fetch_array($getBooster);
	// var_dump($rowBooster);

	//echo "SELECT `employee`.`email` as `emp_email`,`employee`.`name`,`employee`.`mobile` as `EmployeeMobile` FROM `employee` INNER JOIN `contact` ON `employee`.`id`=`contact`.`ownerid` WHERE `contact`.`ownerid`='$empid'";
...
$getEmp=mysql_query("call getEmployee()") or die(mysql_error($con));
