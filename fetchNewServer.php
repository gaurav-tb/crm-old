...
//echo $k = $k-1;
	...
	else if($leadsource=='JPG' && $ownerID=='1' && $code == '')
	{
	$source = '88';//JPG  leads
	}
	...
	if($source == 30)
	{
		...
		mysql_query("INSERT INTO `contact`(`ownerid`, `fname`, `mobile`, `email`, `leadstatus`, `leadsource`, `latestresponse`, `contactstatus`, `callbackdate`,  `city`, `product`, `alloted`, `converted`, `mark`, `id`, `createdate`, `modifieddate`, `updatedby`, `delete`,`notified`,`inroducer`,`%brokerage`) VALUES ('$mates', '$name', '$mobile', '$email', '3', '$source', '1', '1', '$date', '1', '1', '0', '0', '0', '', '$datetime', '$datetime', '$loggeduserid', '0','1','$code','$brokerage')",$con) or die(mysql_error());
	}
	else
	{
	if($code != '')
    ...
    // $getCallbackTemplate = mysql_query("SELECT templateemail,subject FROM `templateemail` WHERE id = '91'",$con) or die(mysql_error());
    // $rowcallbackTemplate = mysql_fetch_array($getCallbackTemplate);

    // $template=$rowcallbackTemplate[0];
    // $subject=$rowcallbackTemplate[1];

    //  $templateemail = str_ireplace("{{employee_name}}",$loggedname,$templateemail);
    //  $templateemail = str_ireplace("{{mobile_number}}",$row['mobile'],$templateemail);

//     // $loggedemail
//     $params = Array();
//     $params['content'] = $templateemail;
//     $params['recipients'] = $loggedemail;
...
// curl_close($ch);
	...
	{
	$type = "Under JPG Leads";//MissedCall leads	
	}
...
?>
    ...
    if($leadsource=='JPG')
    {
    $sql_jpg=mysql_query("SELECT COUNT(id) FROM `contact` WHERE leadsource = '$leadsource' AND latestresponse = '1' AND ownerid = '$loggeduserid'",$con) or die(mysql_error());

    $count_jpg =  mysql_fetch_array($sql_jpg);
