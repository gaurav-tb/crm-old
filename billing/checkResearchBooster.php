...
$getBooster = mysql_query("SELECT * FROM  `researchbooster` WHERE  `cid` =  '$cid' AND `researchbooster`.`id`='$researchid' AND id = (SELECT MAX( id ) FROM  `researchbooster` WHERE cid =  '$cid' )
...
<tr>
<td align="left" style="width: 100%">Please fill below Details For Research Booster Approval<span style="text-transform:capitalize"><?php echo $_GET['name'];?></span></td>
</td>
</tr>
</table> 
</div>
<div>
	...
	<!--<option <?php // if ($row['FundAvailable']==2)  { echo 'Selected';   }?> value="2">Excess Stocks With POA</option>  -->
		...
		<?php  
       // $getMax = mysql_query("SELECT * FROM  `researchbooster` WHERE  `cid` =  '$cid' AND (`Approved`='1' && `ActivationRequest`='0') AND id = (SELECT MAX( id ) FROM  `researchbooster` WHERE cid =  '$cid' )",$con) or die(mysql_error());
		...
		<td><input id='optt5' class="inputCalender" value=""  onclick="openCalendar(this);"  placeholder="YYYY-MM-DD"   type="text" style="display: none" />
			<span id="FundClearDate" title="Fund Clear" style="font-size:9px;"></span>
		</td>
		</tr> 
		<?php //} ?>
		...
		<td>
		<input name="Button2" id="optt7" type="button" value="Click to Approve" class="buttonGreen"  onclick="UpdateResearchApproval('<?php  echo $cid;?>','<?php echo $researchid;?>','6','1');" />
		</td>
		</tr>	
		...
		<td>
		<input name="Button2" id="optt7" type="button" value="Approve Free Trial" class="buttonGreen"  onclick="UpdateResearchApproval('<?php  echo $cid;?>','6','2');" />
		</td>
		</tr>	
		...
		$getCount=mysql_query("SELECT * FROM `researchbooster` WHERE cid='$cid' AND `service`='2' AND `Approved`='2'",$con) or die(mysql_error());
		...
		<td><input class="input" readonly  value="<?php echo $row['Activationamt']?>"  type="text" /></td>
		</tr>
