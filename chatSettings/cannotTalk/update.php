...
mysql_query("UPDATE `cannottalkto` SET `userid` = '$post[0]', `cannottalkto` = '$post[1]', `desc` = '$post[2]', `modifieddate` = '$datetime', `updatedby` = '$loggeduserid' WHERE `id` = '$id'",$con) or die(mysql_error());
...
<tr id="fetchRow<?php echo $i;?>" class="d<?php echo $i%2;?>" title="<?php echo 'Last Updated On '.date("d M,y h:i:s",strtotime($row[3]));?>">
		<td style="width: 20px;">
		<input id="chBx<?php echo $id;?>" name="Checkbox1" type="checkbox" value="<?php echo $row[0];?>" /></td>
