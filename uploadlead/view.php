...
$getCount = mysql_query("SELECT COUNT(`id`) FROM `contact` WHERE `leadsource` = '$rowCity[1]' AND `ownerid` = '0' AND `delete` = '0' AND alloted='0' AND `contact`.`read`='0'",$con) or die(mysql_error());
...
?>
			...
			<?php
			$getUpload = mysql_query("SELECT `id` FROM `uploadcontact` WHERE `delete` = '0'",$con) or die(mysql_error());
			$rowUpload = mysql_num_rows($getUpload);
			if($rowUpload != 0)
			{
			?>
			<div style="" class="blueSimpletext"><strong> <?php echo $rowUpload?></strong> Records left to be uploaded.</div>
			<?php } ?>
...
<br/>
			...
			?>
			<input name="Button1" class="buttonBlue" type="button" value="+1 Add More" onclick="getModule('uploadlead/new','manipulateContent','viewContent','Upload Leads')" />
...
<select name="Select1" class="input" style="width: 390px" id="TeamId">
			
<option value="">Select Team</option>
<?php
$getTeam = mysql_query("SELECT name, id FROM team WHERE team.delete =  '0' ORDER BY name ASC ",$con) or die(mysql_error());
while($rowTeam= mysql_fetch_array($getTeam))
{
?>
<option value="<?php echo $rowTeam[1];?>"><?php echo $rowTeam[0];?></option>
<?php
}
?>

</select>&nbsp;&nbsp;
			<input name="Button1" type="button" value="Get Users" class="buttonBlue" onclick="getModule('uploadlead/customView?TeamId='+document.getElementById('TeamId').value+'&lsid=<?php echo $lsid;?>','customResult','','Fetching Data')" />
