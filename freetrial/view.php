...
$getData = mysql_query("SELECT employee.name, contact.fname, contact.lname, contact.mobile, product.name, servicecall.fromdate, servicecall.todate, contact.id,servicecall.id,category.name,category.id FROM employee,contact,servicecall,product,category WHERE product.category = category.id AND contact.ownerid = employee.id AND product.id = servicecall.product and servicecall.cid = contact.id AND servicecall.approved = '0' AND servicecall.type = 'f' ORDER BY servicecall.createdate DESC",$con) or die(mysql_error());
...
{
//echo "SELECT `id` FROM `servicecall` WHERE `product` = '$row[10]' AND `cid` = '$row[7]' AND `type` = 'f'";
$getCount = mysql_query("SELECT COUNT(`id`) FROM `servicecall` WHERE `product` = '$row[10]' AND `cid` = '$row[7]' AND `type` = 'f'",$con) or die(mysql_error());
