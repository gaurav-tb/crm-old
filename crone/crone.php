...
$getclient = mysql_query("SELECT `email` FROM `contact` WHERE `converted` = '1'",$con) or die(mysql_error());

while($clientdata = mysql_fetch_array($getclient)){

	$email = $clientdata['email'];
	$templateemail = file_get_contents("http://192.168.0.162/crone/email_template.html");
	$params = Array();
