...
,contact.bankname,contact.bankbranchname,contact.bankaccounttype,contact.bankaccountnumber,contact.dpname,contact.dpid,contact.inroducer,contact.clientid,contact.phone,contact.altemail,`contact`.`ownerid`,`customersupport`.`allotmentid`,contact.`paymethod`,contact.firstTradeDate,`customersupport`.`BoMobileNumber2`,`customersupport`.`LastRMOwnerChangeDate`,contact.inroducer,customersupport.lastTradeDate,customersupport.fund_counted,customersupport.fund_counted_date	
...
if($mbl !='')
{
	$sql .= " and contact.mobile IN($getmbl)";	
}
//echo $sql;	
if($fromdate != '' || $todate != '' )
{
...
//$sql .= " and contact.converted = '$contactstatus'";
	$sql .= " and contact.converted IN (0,1)";
}else {
	$sql .= " and contact.converted = '$contactstatus'";
}
//echo $sql;die;
...
$name = "General_Report_".$format.".xls";
...
<th style="height:29px">BoMobileNumber2</th>
<?php } ?>
...
<th style="height:29px">Original Source</th>
...
<th style="height:29px">Last RM Owner Change Date</th>
<?php }?>
...
?>
	<tr>
		...
		<td>
		<?php echo date('d-M-Y',strtotime($row[5]));?>
		</td>
		<td>
		...
		<td>
		<?php echo date('d-M-Y',strtotime($row[13]));?>
		</td>
		<td>
		<?php echo $row[14];?>
		</td>
		...
		<td>
		<?php echo date('d-M-Y',strtotime($row[16]));?>
		</td>
		<td>
		<?php echo date('d-M-Y',strtotime($row[17]));?>
		</td>
		<td>
		...
		<td>
		<?php echo date('d-M-Y',strtotime($row[22])) ;?>
		</td>
		<td>
		<?php echo date('d-M-Y',strtotime($row[23])) ;?>
		</td>
		<td>
		<?php echo $bankmapping[$row[24]] ;?>
		</td>
		<td>
		<?php echo date('d-M-Y',strtotime($row[25])) ;?>
		</td>
		<td>
		<?php echo date('d-M-Y',strtotime($row[26])) ;?>
		</td>
		<td>
		<?php echo date('d-M-Y',strtotime($row[27])) ;?>
		</td>
		<td>
		...
		{
		$activationDate=date('d-M-Y',strtotime($row['conversiondate']));	
		}
		else
		{
		$activationDate=date('d-M-Y',strtotime($row['ApprovedDate']));
		}
...
<td><?php echo date('d-M-Y',strtotime($row['firstTradeDate'])); ?></td>
<?php 
if($contactstatus == '1'){?>
<td><?php echo date('d-M-Y',strtotime($row['LastRMOwnerChangeDate'])); ?></td>
<?php }
?>	
