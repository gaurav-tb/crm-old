...
$query = mysql_query("SELECT contact.fname,contact.lname,contact.mobile,category.name,servicecall.id,servicecall.type FROM contact,servicecall,product,category WHERE servicecall.cid = contact.id AND servicecall.approved = '1' AND ".$cstr." AND ".$tystr." AND contact.delete = '0' AND servicecall.fromdate <= '$fordate' AND servicecall.todate >= '$fordate'  ORDER BY category.name ASC",$con) or die(mysql_error());
...
$masterkey = $row[3].$row[2];
	...
	$master[$masterkey] = 'occured';
		...
		$service[$row[3]] .= "<td>".$row[0]."</td><td>".$row[2]."</td><td>".$type."</td>:::::::forbreak::::::"; 
