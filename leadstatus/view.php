...
$getData = mysql_query("SELECT leadstatus.name,leadstatus.modifieddate,employee.name,leadstatus.id,leadstatus.description FROM leadstatus,employee WHERE leadstatus.updatedby = employee.id AND leadstatus.delete = '0' AND leadstatus.id != '1' ORDER BY leadstatus.id DESC LIMIT 100",$con) or die(mysql_error());
?>
...
?>
	<tr id="fetchRow<?php echo $i;?>" class="d<?php echo $i%2;?>">
		...
		<td onclick="getModule('masters/leadstatus/edit?id=<?php echo $row[3];?>&amp;i=<?php echo $i;?>','manipulateContent','viewContent','Fetching Data..')" style="width: 300px;">
		<?php echo $row[0];?></td>
